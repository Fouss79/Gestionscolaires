...
public class EmargeController {
    ...
    private void loadEnseignants() {
        ...
        String sql = "SELECT id, nom FROM enseignant";
    ...
    private void loadEmploiDuTemps() {
        ...
        try (Connection conn = Database.connect();
            ...
            ps.setString(1, cbEnseignant.getValue());
            ps.setInt(2, ContexteApplication.getAnneeId());
    ...
    private void loadEmploiDuTempsParJour(String jour) {
        ...
        if (anneeId == null) {
            System.out.println("Aucune année active !");
            return;
        }

        String sql = """
            SELECT e.*, a.libelle AS annee_libelle
	...
	private void loadEmargementsDuJour(String enseignantId, LocalDate date) {
	    ...
	    try (Connection conn = Database.connect();
	        ...
	        ps.setInt(1, Integer.parseInt(enseignantId));
	        ps.setString(2, date.toString());
	        ps.setInt(3, ContexteApplication.getAnneeId());
	        ...
	        while (rs.next()) {
	            ...
	            Emargement e = new Emargement(
	                ...
	                rs.getString("classe"),
	                rs.getString("matiere"),
	                LocalDate.parse(rs.getString("date_heure")),
	                rs.getInt("duree"),
	...
	private void loadEmargementsDuJourParjour(String jour, LocalDate date) {
	    ...
	    String sql = "SELECT * FROM emargement WHERE jour = ? AND date_heure = ?";

	    try (Connection conn = Database.connect();
	         PreparedStatement ps = conn.prepareStatement(sql)) {

	        ps.setString(1, jour);
	        ps.setString(2, date.toString()); // format YYYY-MM-DD

	        ResultSet rs = ps.executeQuery();

	        while (rs.next()) {

	            // 🔥 Lire la date directement
	            ...
	            Emargement e = new Emargement(
	                    rs.getInt("id"),
	                    rs.getString("jour"),
	                    rs.getString("enseignant_id"), // ⚠️ normalement c’est un int
	                    rs.getString("enseignant"),
	                    rs.getString("classe"),
	                    rs.getString("matiere"),
	                    localDate,   // 🔥 plus de LocalDateTime
	                    rs.getInt("duree"),
	                    rs.getBoolean("present")
	            );
       ...
       @FXML private void openEleve(ActionEvent event) { changerScene(event,"ElevesView.fxml", "Gestion des Élèves"); }
       ...
       void openHabilitation(ActionEvent event) {
    	   changerScene(event,"HabilitationView.fxml", "Habiitation");
       }
         ...
         if (selected.getProfesseur().contains(" - ")) {
           ...
           try (Connection conn = Database.connect()) {
        	...
        	try (PreparedStatement checkPs = conn.prepareStatement(checkSql)) {
            	   checkPs.setString(1, ensNom);
            	   checkPs.setString(2, selected.getClasse());
            	   checkPs.setString(3, selected.getMatiere());
            	   checkPs.setString(4, selectedDate.toString());
            	   checkPs.setInt(5, ContexteApplication.getAnneeId());
               ...
               String insertSql = """
            		    ...
            		    (enseignant_id, jour, enseignant, classe, matiere, date_heure, duree, present, annee_id)
            		    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            		""";
            		...
            		try (PreparedStatement ps = conn.prepareStatement(insertSql)) {
            		    ...
            		    ps.setString(6, selectedDate.toString());
            		    ps.setInt(7, duree);
            		    ps.setBoolean(8, true);
            		    ps.setInt(9, ContexteApplication.getAnneeId()); // 🔥 IMPORTANT
