...
public class EmargeController {
        ...
        try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD);
            ...
            while (rs.next()) {
            	EmploiDutemps edt = new EmploiDutemps(
            	        ...
            	        rs.getString("classe"),
            	        rs.getInt("annee_id"),          // ⚠️ doit exister dans la requête SQL
            	        rs.getString("annee_libelle")   // si tu fais un JOIN avec table annee
            	);
                emplois.add(edt);
            }
	...
	private void loadEmploiDuTempsParJour(String jour) {
        emplois.clear();
        String sql = "SELECT e.*, a.libelle AS annee_libelle\r\n"
         ...
         if (selected.getProfesseur().contains(" - ")) {
           ...
           try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)) {
               ...
               String checkSql = """
                   ...
                   WHERE enseignant = ? AND classe = ? AND matiere = ? AND DATE(date_heure) = ?
               """;
